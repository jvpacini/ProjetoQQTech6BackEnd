...
const getFuncoes = async (req, res) => {
  try {
    const funcoes = await getAllFuncoes();
...
const editFuncao = async (req, res) => {
  try {
    const { codigo_funcao, nome_funcao, descricao } = req.body;
    const updatedFuncao = await updateFuncao(
      req.params.id,
      codigo_funcao,
      nome_funcao,
      descricao
...
module.exports = { getFuncao, getFuncoes, addFuncao, editFuncao, removeFuncao };
